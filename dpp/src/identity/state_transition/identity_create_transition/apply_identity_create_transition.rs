        ...
        SR: StateRepositoryLike,
{
    pub async fn apply_data_contract_create_transition(
        ...
        let output = self.asset_lock_transaction_output_fetcher.fetch(state_transition.get_asset_lock_proof()).await?;
        ...
        self.state_repository.create_identity(&identity);
        ...
        self.state_repository.store_identity_public_key_hashes(
            identity.get_id(),
            public_key_hashes,
        );

        let out_point = state_transition.get_asset_lock_proof().out_point().ok_or_else(|| anyhow!("Out point is missing from asset lock proof"))?;

        self.state_repository.mark_asset_lock_transaction_out_point_as_used(&out_point);
